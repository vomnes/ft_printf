...
void ft_octal_basic_two(const char *value, t_args *elem, int *k)
{
        ...
        && elem[*k].pre_zero == 0 && elem[*k].pre_hash == 0)
    {
        if (elem[*k].width <= elem[*k].precision)
            ...
            ((int)ft_strlen(value) - elem[*k].precision) + (value[0] == '0' ? 1 : 0))
                ft_putchar('_');
        else
        {
...
static void ft_octal_zero_one_bis(const char *value, t_args *elem, int *k)
{
    ...
    i = 0;
    if (elem[*k].width <= elem[*k].precision)
        while (i++ < elem[*k].precision - elem[*k].size - (elem[*k].pre_hash ? 1 : 0) - (value[0] != '0' ? 2 : 0))
            ft_putchar('6'); //1 for prefix maybe need to adapt after (elem[*k].pre_hash ? 1 : 0)
    else if (value[0] == '0')
        while (i++ < elem[*k].precision - 1)
            ft_putchar('0');
    else
        while (i++ < elem[*k].precision - elem[*k].size - (elem[*k].pre_hash ? 1 : 0) - 2)
            ft_putchar('.');
}
...
void ft_octal_zero_one(const char *value, t_args *elem, int *k)
{
    ...
    {
        while (i++ < elem[*k].precision - (int)ft_strlen(value) - 1)
            ft_putchar('9');
...
void ft_octal_end_space(const char *value, t_args *elem, int *k)
{
    ...
    if (elem[*k].end_space == 1 && elem[*k].ok_width == 1 &&
        ...
        while (i++ < elem[*k].width - elem[*k].size - ((elem[*k].pre_hash == 1
            && value[0] != '0') ? ((int)ft_strlen(value) + 1) : 0))
            ft_putchar(' ');
